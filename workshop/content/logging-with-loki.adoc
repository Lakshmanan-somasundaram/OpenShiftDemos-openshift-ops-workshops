...
====
  ...
  b. Type `OpenShift Logging` in the search box and click the  `Red Hat OpenShift Logging` card from the list of available Operators (choose 'stable-5.9') (please do not select `stable-6.0`), and click `Install`.
  c. On the `Install Operator` page, *select Update Channel `stable 5.9` and  and `Version 5.9.7` *. Under `Installation Mode` ensure that `A specific namespace on the cluster` is selected, and it should say that the
...
2. Install the `Loki Operator`:
  a. In the OpenShift console, click `Operators` → `OperatorHub`.
  b. Type `Loki Operator` in the search field and click the `Loki Operator` card from the list of available Operators (choose `stable-5.9 and Version '5.9.7') (please do not select `stable-6.0`), and then click `Install`.
